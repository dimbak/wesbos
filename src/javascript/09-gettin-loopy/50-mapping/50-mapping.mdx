...
![calling addArms funtion and passing an emoji as argument](./631.png)
...
![taking array of faces and adding arms to all of them using array map method](./632.png)
...
![taking an array of names and transforming names by adding string bos at the end ](./633.png)
...
![making a separate function named bosify and passing it as callback to map method](./634.png)
...
![accessing each character of a word using an index](./636.png)
...
![chaining maps to produce first character as capitalize of the fullNames array](./638.png)
...
![using slice method to fix the issue with chain of maps which only returns first character](./639.png)
...
![replacing string concatination operator with template literals to use variables inside string](./640.png)
...
![returning 1 to every single item of orderTotals array](./641.png)
...
![creating a new array named totalWithTax by multiplying 1.13 to every item of the orderTotals array](./642.png)
...
![creating cowboy bodies using emojis](./643.png)
...
![calling repeat method on a string](./644.png)
...
![creating array of 3 items using Array keyword](./645.png)
...
![filling the array with elements using array.fill method](./646.png)
...
![generating current date time using date method](./647.png)
...
![generating a date by passing a string of date to date method](./648.png)
...
![calculating how old is the person is using birth date and current date](./649.png)
...
![getTime method of date returning milliseconds since January 1, 1970](./650.png)
...
![shorthand way to get the timestamp using Date.now()](./651.png)

If you ever have one of these timestamps, you can go to the website https://epoch.now.sh.

![converting timestamps to date using https://epoch.now.sh](./652.png)
...
![difference between two dates](./653.png)
...
![calculate the difference between two dates to find the age of a person](./654.png)
...
![rounding of the age of person using math.floor()](./655.png)
...
![displaying cleanPeople in a table to make it easy to understand](./656.png)
